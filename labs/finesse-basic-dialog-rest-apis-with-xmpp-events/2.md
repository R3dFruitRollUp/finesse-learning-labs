...
1. Open **Chrome**, click the **Apps** button on the shortcut toolbar, and select **Postman**.
 ...
 * In the field marked *Enter request URL here*, enter the URL for the Finesse API ``/SystemInfo`` resource: ``http://{{web-url}}:8082/finesse/api/SystemInfo``
 * From the request method dropdown menu, select **GET**.

	![SystemInfo GET Request](/posts/files/finesse-basic-dialog-rest-apis-with-xmpp-events/assets/images/systemInfo-GET-request.jpg)
4. Click **Send** to launch the request.
5. If the request is successful, you will see a HTTP status of 200 OK and see some XML formatted data with the status of the Finesse system.

 ![SystemInfo GET Response](/posts/files/finesse-basic-dialog-rest-apis-with-xmpp-events/assets/images/systemInfo-GET-response.jpg)
