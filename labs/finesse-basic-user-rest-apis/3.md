## Step 3: Get information about userA
 ...
 * For the Basic Auth **password** value, enter UserA's password: ``{{agentA_password}}``
    ...
    ![UserA GET Request Before Login](/posts/files/finesse-basic-user-rest-apis/assets/images/user-a-GET-request-before-login.jpg)
 * Click **Update request** to generate an authorization header.
4. Click **Send** to launch the request.
5. If the request is successful, you will get a HTTP status of 200 OK and see some XML formatted data with information about the user, such as user first name, last name, and user ``<state>`` tag value of LOGOUT.

 ![UserA GET Response Before Login](/posts/files/finesse-basic-user-rest-apis/assets/images/user-a-GET-response-before-login.jpg)
6. Since this step is repeated throughout this learning lab, keep the Postman tab open.
